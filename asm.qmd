...
![](images/china-emissions.png){fig-align="center" width="80%"}
:::
...
![](images/china-production.png){fig-align="center" width="80%"}
:::
:::
...
![Caihuying Villagers' Committee](images/village-committee.jpg){width="80%"}
:::
...
::: aside
Image credit: N509FZ, CC BY-SA 4.0 via [Wikimedia Commons](https://commons.wikimedia.org/w/index.php?curid=103958204)
:::
...
:::{.r-stack}
![](images/data-collection-flow.png){.fragment fig-align="center" width="80%"}

![](images/Participation-flowchart-Apr12.png){.fragment}
:::
...
$$Y_{ijt}= \alpha + \sum_{r=q}^{T} \beta_{r} d_{r} + \sum_{s=r}^{T} \gamma_{s} fs_{t}+ \sum_{r=q}^{T} \sum_{s=r}^{T} \tau_{rs} (d_{r} \times fs_{t}) + \mathbf{X}_{ijt} + \varepsilon_{ijt}$$

- $d_{r}$ = treatment cohort fixed effects  
- $fs_{t}$ = time fixed effects  
- $\tau_{rt}$ = cohort-time *ATTs*
- $\mathbf{X}_{ijt}$ = time-varying covariates
...
$$\quad + \delta M_{it} + \sum_{r=q}^{T} \sum_{s=r}^{T} \eta_{rs} (d_{r} \times fs_{t} \times M_{it}) + \zeta \mathbf{W} + \varepsilon_{ijt}$$

where now we have added: 

- $\delta$ = conditional effect of mediator  
- $\eta_{rs}$ = treatment-mediator product terms
...
Little evidence of impact on inflammatory markers or FeNO.
...
##
:::{#title-slide .center}
[**Do PM~2.5~ and temperature mediate the BP effect?**]{.large}
:::
...
## Uptake
high uptake and consistent use of the new heat pump technology, as well as large reductions in coal use in treated villages

## Summary

 - Strong uptake of the intervention via transitions to heating with electricity or combined electricity + solid fuel
 - Impacts on indoor PM2.5 but not personal exposures or outdoor PM2.5
 - Initial source apportionment suggests useful variation by treatment and time
 - Overall lower BP but heterogeneous effects by treatment group
 - Improvements in respiratory symptoms, roughly 10-25% mediated by PM2.5
